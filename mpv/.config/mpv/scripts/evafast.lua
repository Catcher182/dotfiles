...
local function adjust_speed()
	...
	if speedup_target ~= nil then
		...
		if current_time >= speedup_target then
			...
			local time_for_correction = speed_transition(
				speed,
				math.max(
					math.min(options.speed_cap, options.subs_speed_cap and options.subs_speed_cap or options.speed_cap),
					1.1
				)
			) -- not effective_speed_cap because it may lead to huge fluctuations in transition speed
	...
	if speed ~= old_speed then
		...
		if
			(options.show_speed and not toggle_display)
			or (options.show_speed_toggled and toggle_display and speedup_target == nil)
			or (options.show_speed_target and speedup_target ~= nil)
		then
...
local function evafast(keypress)
	...
	if keypress["event"] == "up" or keypress["event"] == "press" then
		...
		if
			speed_timer ~= nil
			and not toggle_state
			and mp.get_property_number("speed") == 1
			and (
					(not options.subs_speed_cap or mp.get_property("sub-start") == nil) and options.speed_cap
					or options.subs_speed_cap
				)
				~= 1
		then
			speed_timer:kill()
...
mp.add_key_binding("Ctrl+SPACE", "evafast", evafast, { repeatable = true, complex = true })
