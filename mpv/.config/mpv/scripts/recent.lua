...
function update_dyn_menu_items()
	...
	for i = 1, length do
		menu[#menu + 1] = {
			title = string.format(
				"%s\t%s",
				o.show_paths and strip_title(split_ext(get_filename(lists[#lists - i + 1])))
					or strip_title(split_ext(lists[#lists - i + 1].title)),
				get_ext(lists[#lists - i + 1].path)
			),
...
function draw_list(list, start, choice)
	...
	local msg = string.format("{\\fscx%f}{\\fscy%f}{\\bord%f}", font_scale, font_scale, o.border_size)
